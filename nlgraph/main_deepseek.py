...
def work(args):
    if not os.path.exists("./log_deepseek"):
        os.mkdir("./log_deepseek")
    logger = new_log("./log_deepseek", args)
            ...
            try:
                ...
                if dataset_name in pred_less_groundtruth:
                    ...
                    if exact_answer > 0:
                        ratio.append(abs(result - exact_answer) / abs(exact_answer))
    t2 = time.time()
    PRINT_LOG(f"average_time: {(t2 - t1) / tot_number:.4f}s")
